      ...
      <main className="main-content">
        <section className="video-section">
          <div className="video-wrapper">
            {streamerData ? (
              <iframe
                src={`https://customer-${accountId}.cloudflarestream.com/${streamerData.cloudflare_uid}/iframe?preload=true&autoplay=true&muted=true`}
              ...
              <div className="no-stream">
                ...
                <p>Este streamer aún no ha generado su canal.</p>
              </div>
            )}
          </div>
          ...
          <div className="details-bar">
            <div className="streamer-info">
              ...
              <div className="meta">
                <h3>{username}</h3>
                <p>Streaming en directo • 1080p Ultra</p>
              </div>
            </div>
              ...
              {recommendedStreams.map(stream => (
                <div key={stream.id} className="rec-card" onClick={() => window.location.href = `/stream/${stream.name.toLowerCase()}`}>
                  ...
                  <div className="rec-info">
                    <h4>{stream.title}</h4>
                    <p>{stream.name} • {stream.viewers} viewers</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <aside className="sidebar">
          <div className="ad-box-sidebar">
            <iframe srcDoc={adSidebarHtml} width="100%" height="250" style={{ border: 'none' }} sandbox="allow-scripts allow-forms allow-same-origin" />
          </div>
            ...
            <div className="chat-messages">
              {messages.map((msg) => (
                <div key={msg.id} className="msg">
                  <span className={msg.mod ? 'mod' : 'user-name'}>{msg.user}:</span> {msg.text}
                </div>
              ))}
            </div>
          ...
          <div className="ad-box-sidebar">
            <iframe 
              src="https://www.profitablecpmratenetwork.com/dnhcmvse?key=2a3357726d5ea2f9e1aad0f54a5ca4db" 
              width="100%" 
              height="250" 
